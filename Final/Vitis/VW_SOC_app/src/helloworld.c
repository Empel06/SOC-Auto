#include "xparameters.h"
#include "xgpio.h"
#include "xtmrctr.h"
#include "xscugic.h"
#include "xil_exception.h"
#include "ultrasoon.h"
#include "SpeedSensor.h"
#include "xiic.h"


/******************* GPIO DEFINITIONS *******************/
#define GPIO_DEVICE_ID       XPAR_AXI_GPIO_0_DEVICE_ID
#define GPIO_CHANNEL         1

// GPIO pin numbers based on the constraints file
#define IN1_PIN              0 // GPIO_0_tri_io[0] is connected to pin Y11
#define IN2_PIN              1 // GPIO_0_tri_io[1] is connected to pin Y12
#define IN3_PIN              2 // GPIO_0_tri_io[2] is connected to pin W11
#define IN4_PIN              3 // GPIO_0_tri_io[3] is connected to pin V11

XGpio Gpio; // GPIO driver instance

/******************* PWM DEFINITIONS *******************/
#define SCUGIC_DEVICE_ID        XPAR_SCUGIC_SINGLE_DEVICE_ID
#define AXI_TIMER_DEVICE_ID     XPAR_AXI_TIMER_0_DEVICE_ID
#define AXI_TIMER_IRPT_INTR     XPAR_FABRIC_AXI_TIMER_0_INTERRUPT_INTR

XTmrCtr xTmrCtr_Inst;
XScuGic xScuGic_Inst;

#define AXI_TIMER_CHANNEL_1     0
#define AXI_TIMER_CHANNEL_2     1

#define AXI_TIMER_PWM_TEST_SW   1

#define AXI_TIMER_PERIOD_US     10000
#define AXI_TIMER_PWM_HIGH_TIME_US 10000

/******************* ULTRASONIC AND SPEED SENSOR DEFINITIONS *******************/
#define ULTRASONIC_BASEADDR_0   XPAR_ULTRASOON_0_S00_AXI_BASEADDR
#define ULTRASONIC_BASEADDR_1   XPAR_ULTRASOON_1_S00_AXI_BASEADDR
#define SPEEDSENSOR_BASEADDR_0 XPAR_SPEEDSENSOR_0_S00_AXI_BASEADDR
#define SPEEDSENSOR_BASEADDR_1 XPAR_SPEEDSENSOR_1_S00_AXI_BASEADDR

#define TURN_THRESHOLD_CM 5
#define TURN_COUNT_THRESHOLD 2

/******************* IMU DEFINITIONS *******************/
#define MPU6050_I2C_ADDRESS 0x68
#define WHO_AM_I_REGISTER 0x75
#define M_PI 3.14159265358979323846

/******************* HELPER FUNCTIONS *******************/
void xTmrCtr_Int_Handler(void *CallBackRef, u8 TmrCtrNumber) {
    XTmrCtr *InstancePtr = (XTmrCtr *)CallBackRef;
    u32 CounterValue = XTmrCtr_GetValue(InstancePtr, TmrCtrNumber);
}

u32 xTmr_US_To_RegValue(u32 US) {
    return 0xFFFFFFFF - (50 * US);
}

u32 xTmr_US_To_NS(u32 US) {
    return US * 1000;
}

/******************* IMU CONTROLLER *******************/
u8 who_am_i = 0;
u8 MPU_init[] = {0x6B, 0};     // Wake up device
u8 MPU_read[] = {0x3B};        // Start reading from accel

int16_t AcX, AcY, AcZ, Tmp, GyX, GyY, GyZ;
int AcXcal = -950, AcYcal = -300, AcZcal = 0;
int tcal = -1600;
int GyXcal = 480, GyYcal = 170, GyZcal = 210;
double t, tx, pitch, roll;

u8 DataBuffer[14];
u8 who_am_i_reg = WHO_AM_I_REGISTER;

// Bereken pitch en roll vanuit accelerometerwaarden
void getAngle(int Ax, int Ay, int Az) {
    pitch = atan((double)Ax / sqrt(Ay * Ay + Az * Az)) * 180.0 / M_PI;
    roll = atan((double)Ay / sqrt(Ax * Ax + Az * Az)) * 180.0 / M_PI;
}

// Initialiseer de IMU en check WHO_AM_I
int init_imu() {
    // Vraag WHO_AM_I op
    XIic_Send(XPAR_IIC_0_BASEADDR, MPU6050_I2C_ADDRESS, &who_am_i_reg, 1, XIIC_REPEATED_START);
    XIic_Recv(XPAR_IIC_0_BASEADDR, MPU6050_I2C_ADDRESS, &who_am_i, 1, XIIC_STOP);

    xil_printf("WHO_AM_I register value: 0x%X\n\r", who_am_i);

    if (who_am_i != 0x68) {
        xil_printf("MPU-6050 niet gedetecteerd. Controleer bedrading en I2C adres.\n\r");
        return -1;
    }

    // Initialisatiecommando sturen
    XIic_Send(XPAR_IIC_0_BASEADDR, MPU6050_I2C_ADDRESS, MPU_init, 2, XIIC_STOP);

    xil_printf("MPU-6050 correct geïnitialiseerd.\n\r");
    return 0;
}

// Lees waarden uit de IMU en bereken hoeken
void read_imu() {
    XIic_Send(XPAR_IIC_0_BASEADDR, MPU6050_I2C_ADDRESS, MPU_read, 1, XIIC_REPEATED_START);
    XIic_Recv(XPAR_IIC_0_BASEADDR, MPU6050_I2C_ADDRESS, DataBuffer, 14, XIIC_STOP);

    AcX = (DataBuffer[0] << 8) | DataBuffer[1];
    AcY = (DataBuffer[2] << 8) | DataBuffer[3];
    AcZ = (DataBuffer[4] << 8) | DataBuffer[5];
    Tmp = (DataBuffer[6] << 8) | DataBuffer[7];
    GyX = (DataBuffer[8] << 8) | DataBuffer[9];
    GyY = (DataBuffer[10] << 8) | DataBuffer[11];
    GyZ = (DataBuffer[12] << 8) | DataBuffer[13];

    tx = Tmp + tcal;
    t = tx / 340 + 36.53;

    getAngle(AcX, AcY, AcZ);
}


/******************* PWM CONTROLLER *******************/
int xTmrCtr_Init(XTmrCtr *xTmrCtr_Ptr, u32 DeviceId) {
    int Status, DutyCycle;

    Status = XTmrCtr_Initialize(xTmrCtr_Ptr, DeviceId);
    if (Status != XST_SUCCESS) {
        return XST_FAILURE;
    }

    Status = XTmrCtr_SelfTest(xTmrCtr_Ptr, 2 - 1);
    if (Status != XST_SUCCESS) {
        return XST_FAILURE;
    }

    XTmrCtr_SetHandler(xTmrCtr_Ptr, xTmrCtr_Int_Handler, xTmrCtr_Ptr);
    XTmrCtr_SetResetValue(xTmrCtr_Ptr, AXI_TIMER_CHANNEL_1, xTmr_US_To_RegValue(AXI_TIMER_PERIOD_US));
    XTmrCtr_SetResetValue(xTmrCtr_Ptr, AXI_TIMER_CHANNEL_2, xTmr_US_To_RegValue(AXI_TIMER_PERIOD_US));
    XTmrCtr_SetOptions(xTmrCtr_Ptr, AXI_TIMER_CHANNEL_1, XTC_INT_MODE_OPTION | XTC_AUTO_RELOAD_OPTION);
    XTmrCtr_SetOptions(xTmrCtr_Ptr, AXI_TIMER_CHANNEL_2, XTC_INT_MODE_OPTION | XTC_AUTO_RELOAD_OPTION);

#if AXI_TIMER_PWM_TEST_SW
    DutyCycle = XTmrCtr_PwmConfigure(xTmrCtr_Ptr, xTmr_US_To_NS(AXI_TIMER_PERIOD_US), xTmr_US_To_NS(AXI_TIMER_PWM_HIGH_TIME_US));
#endif

    return XST_SUCCESS;
}

void SetMotorSpeed(int high_time_us) {
    XTmrCtr_SetResetValue(&xTmrCtr_Inst, AXI_TIMER_CHANNEL_2, xTmr_US_To_RegValue(high_time_us));
}

/******************* INTERRUPT CONTROLLER *******************/
int xScuGic_Init(XScuGic *ScuGic_Ptr, XTmrCtr *xTmrCtr_Ptr) {
    int Status;
    XScuGic_Config *intc_cfg_ptr;

    intc_cfg_ptr = XScuGic_LookupConfig(SCUGIC_DEVICE_ID);
    if (intc_cfg_ptr == NULL) {
        return XST_FAILURE;
    }

    Status = XScuGic_CfgInitialize(ScuGic_Ptr, intc_cfg_ptr, intc_cfg_ptr->CpuBaseAddress);
    if (Status != XST_SUCCESS) {
        return XST_FAILURE;
    }

    Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_INT, (Xil_ExceptionHandler)XScuGic_InterruptHandler, ScuGic_Ptr);
    Xil_ExceptionEnable();

    Status = XScuGic_Connect(ScuGic_Ptr, AXI_TIMER_IRPT_INTR, (Xil_ExceptionHandler)XTmrCtr_InterruptHandler, (void *)xTmrCtr_Ptr);
    if (Status != XST_SUCCESS) {
        return XST_FAILURE;
    }

    XScuGic_Enable(ScuGic_Ptr, AXI_TIMER_IRPT_INTR);

    return XST_SUCCESS;
}

/******************* MOTOR CONTROL *******************/
void Motor_Forward() {
    XGpio_DiscreteWrite(&Gpio, GPIO_CHANNEL, (1 << IN2_PIN) | (1 << IN3_PIN));
}

void Motor_Right() {
	XGpio_DiscreteWrite(&Gpio, GPIO_CHANNEL, (1 << IN3_PIN));
}

void Motor_Left() {
    XGpio_DiscreteWrite(&Gpio, GPIO_CHANNEL, (1 << IN2_PIN));
}

/******************* ULTRASONIC SENSOR *******************/
void Ultrasonic_Init() {}

unsigned int Get_Ultrasonic_Distance_0() {
    return ULTRASOON_mReadReg(ULTRASONIC_BASEADDR_0, ULTRASOON_S00_AXI_SLV_REG2_OFFSET);
}

unsigned int Get_Ultrasonic_Distance_1() {
    return ULTRASOON_mReadReg(ULTRASONIC_BASEADDR_1, ULTRASOON_S00_AXI_SLV_REG2_OFFSET);
}

/******************* SPEED SENSOR *******************/
void Speed_Sensor_Init() {
    SPEEDSENSOR_mWriteReg(SPEEDSENSOR_BASEADDR_0, SPEEDSENSOR_S00_AXI_SLV_REG1_OFFSET, 0b1111);
    SPEEDSENSOR_mWriteReg(SPEEDSENSOR_BASEADDR_1, SPEEDSENSOR_S00_AXI_SLV_REG1_OFFSET, 0b1111);
}

unsigned int Get_Speed_Sensor_Value_0() {
    return SPEEDSENSOR_mReadReg(SPEEDSENSOR_BASEADDR_0, SPEEDSENSOR_S00_AXI_SLV_REG2_OFFSET);
}

unsigned int Get_Speed_Sensor_Value_1() {
    return SPEEDSENSOR_mReadReg(SPEEDSENSOR_BASEADDR_1, SPEEDSENSOR_S00_AXI_SLV_REG2_OFFSET);
}

/******************* TURNING FUNCTIONS *******************/
void Turn_Right_Pulses(int pulses) {
    int initial_pulse_count_1 = Get_Speed_Sensor_Value_1();
    int max_attempts = 1000; // Max iteraties
    int attempts = 0;

    while (Get_Speed_Sensor_Value_1() < initial_pulse_count_1 + pulses && attempts < max_attempts) {
        Motor_Left();
        usleep(5000); // 10 ms
        attempts++;

        if (Get_Ultrasonic_Distance_0() < 15 || Get_Ultrasonic_Distance_1() < 15) {
            Motor_Forward();
            initial_pulse_count_1 = Get_Speed_Sensor_Value_1();
        }
    }

    Motor_Forward(); // Ga weer vooruit
}


void Turn_Left_Pulses(int pulses) {
	    int initial_pulse_count_0 = Get_Speed_Sensor_Value_0();
	    int max_attempts = 1000; // Max iteraties
	    int attempts = 0;

	    while (Get_Speed_Sensor_Value_0() < initial_pulse_count_0 + pulses && attempts < max_attempts) {
	        Motor_Right();
	        usleep(5000); // 10 ms
	        attempts++;

	        if (Get_Ultrasonic_Distance_1() < 15 || Get_Ultrasonic_Distance_0() < 15) {
	            Motor_Forward();
	            initial_pulse_count_0 = Get_Speed_Sensor_Value_0();
	        }
	    }

	    Motor_Forward(); // Ga weer vooruit
	}

/******************* MAIN FUNCTION *******************/
int main() {
    int Status;
    int belowThresholdCount = 0;

    // Platform init
    init_platform();
    xil_printf("Autonome auto gestart met obstakeldetectie en IMU\n\r");

    // GPIO init
    Status = XGpio_Initialize(&Gpio, GPIO_DEVICE_ID);
    if (Status != XST_SUCCESS) {
        return XST_FAILURE;
    }
    XGpio_SetDataDirection(&Gpio, GPIO_CHANNEL, ~(1 << IN1_PIN | 1 << IN2_PIN | 1 << IN3_PIN | 1 << IN4_PIN));

    // Timer init
    Status = xTmrCtr_Init(&xTmrCtr_Inst, AXI_TIMER_DEVICE_ID);
    if (Status != XST_SUCCESS) {
        return XST_FAILURE;
    }

    Status = xScuGic_Init(&xScuGic_Inst, &xTmrCtr_Inst);
    if (Status != XST_SUCCESS) {
        return XST_FAILURE;
    }

    XTmrCtr_Start(&xTmrCtr_Inst, AXI_TIMER_CHANNEL_1);
    XTmrCtr_Start(&xTmrCtr_Inst, AXI_TIMER_CHANNEL_2);

#if AXI_TIMER_PWM_TEST_SW
    XTmrCtr_PwmDisable(&xTmrCtr_Inst);
    XTmrCtr_PwmEnable(&xTmrCtr_Inst);
#endif

    // Sensor init
    Ultrasonic_Init();
    Speed_Sensor_Init();

    // IMU init
    if (init_imu() != 0) {
        xil_printf("IMU initialisatie mislukt. Programma wordt gestopt.\n\r");
        cleanup_platform();
        return -1;
    }

    // Start motor
    Motor_Forward();

    // Main loop
    while (1) {
        // --- SPEED SENSOR UITLEZEN ---
        u32 speedLeft = Get_Speed_Sensor_Value_0();
        u32 speedRight = Get_Speed_Sensor_Value_1();
        xil_printf("SpeedSensor: Links = %u, Rechts = %u\n\r", speedLeft, speedRight);

        // --- ULTRASOON SENSOR LOGICA ---
        int distance0 = ULTRASOON_mReadReg(XPAR_ULTRASOON_0_S00_AXI_BASEADDR, ULTRASOON_S00_AXI_SLV_REG2_OFFSET);
        int distance1 = ULTRASOON_mReadReg(XPAR_ULTRASOON_1_S00_AXI_BASEADDR, ULTRASOON_S00_AXI_SLV_REG2_OFFSET);

        xil_printf("Afstanden: L = %d cm, R = %d cm\n\r", distance0, distance1);

        if (distance0 < 5) distance0 = 1000;
        if (distance1 < 5) distance1 = 1000;

        if (distance0 < TURN_THRESHOLD_CM || distance1 < TURN_THRESHOLD_CM) {
            belowThresholdCount++;
            if (belowThresholdCount >= TURN_COUNT_THRESHOLD) {
                if (distance0 < TURN_THRESHOLD_CM) {
                    xil_printf("Object links! Draai rechts.\n\r");
                    Turn_Right_Pulses(15);
                } else if (distance1 < TURN_THRESHOLD_CM) {
                    xil_printf("Object rechts! Draai links.\n\r");
                    Turn_Left_Pulses(15);
                }
                belowThresholdCount = 0;
            }
        } else if ((distance0 >= 25 && distance0 <= 50) || (distance1 >= 25 && distance1 <= 50)) {
            xil_printf("Object tussen 25-50 cm! Vertragen.\n\r");
            SetMotorSpeed(AXI_TIMER_PWM_HIGH_TIME_US / 2);
        } else {
            belowThresholdCount = 0;
            SetMotorSpeed(AXI_TIMER_PWM_HIGH_TIME_US);
            Motor_Forward();
        }

        // --- IMU LOGICA ---
        read_imu();

        xil_printf("Accelerometer: X = %d Y = %d Z = %d\n\r", (AcX + AcXcal), (AcY + AcYcal), (AcZ + AcZcal));
        xil_printf("Gyroscoop: X = %d Y = %d Z = %d\n\r", (GyX + GyXcal), (GyY + GyYcal), (GyZ + GyZcal));

        if (pitch > 20.0) {
            xil_printf("Let op: Steile helling omhoog!\n\r");
            // Voeg gedrag toe zoals vertragen
        } else if (pitch < -20.0) {
            xil_printf("Let op: Steile helling omlaag!\n\r");
            // Voeg gedrag toe zoals aanpassen rijrichting
        }

        usleep(100000);  // 100 ms delay
    }

    cleanup_platform();
    return 0;
}
